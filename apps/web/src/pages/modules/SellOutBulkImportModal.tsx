...
import { bulkImportSellOuts } from '../../store/slices/sellOutSlice';
import { downloadSellOutImportTemplate } from '../../utils/sellOutImportTemplate';
import type { BulkImportResult } from '../../api/sellOut.api';
...
export default function SellOutBulkImportModal({ onClose }: Props) {
  ...
  const handleImport = async () => {
    if (!file) return;
    setLoading(true);
    const res = await dispatch(bulkImportSellOuts(file));
    setLoading(false);
    if (bulkImportSellOuts.fulfilled.match(res)) {
  ...
  return (
    <div>
      <div className="import-template-card">
        ...
        <Button type="button" variant="outline" size="sm" icon={<DownloadIcon />} onClick={() => downloadSellOutImportTemplate()}>
