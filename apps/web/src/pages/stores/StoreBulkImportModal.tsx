...
import { bulkImportStores } from '../../store/slices/storesSlice';
import { downloadStoreImportTemplate } from '../../utils/storeImportTemplate';
import type { BulkImportResult } from '../../api/stores.api';
...
export default function StoreBulkImportModal({ onClose }: Props) {
  ...
  const handleImport = async () => {
    if (!file) return;
    setLoading(true);
    const res = await dispatch(bulkImportStores(file));
    setLoading(false);
    if (bulkImportStores.fulfilled.match(res)) {
  ...
  return (
    <div>
      <div className="import-template-card">
        ...
        <Button type="button" variant="outline" size="sm" icon={<DownloadIcon />} onClick={() => downloadStoreImportTemplate()}>
