...
import { importPromoFile } from '../../store/slices/promosSlice';
import { downloadPromoImportTemplate } from '../../utils/promoImportTemplate';
import type { PromoImportResult } from '../../api/promos.api';
...
export default function ImportPromoModal({ onClose }: Props) {
  ...
  const handleImport = async () => {
    if (!file) return;
    setLoading(true);
    const res = await dispatch(importPromoFile(file));
    setLoading(false);
    if (importPromoFile.fulfilled.match(res)) {
      setResult(res.payload);
      if (res.payload.created > 0) toast.success(t('toasts.importSuccess'));
      if (res.payload.failed === 0 && res.payload.created > 0) {
        setFile(null);
        if (fileInputRef.current) fileInputRef.current.value = '';
      }
    } else {
      toast.error((res.payload as string) || t('toasts.importError'));
    }
  };
  ...
  return (
    <div>
      <div className="import-template-card">
        ...
        <Button type="button" variant="outline" size="sm" icon={<DownloadIcon />} onClick={() => downloadPromoImportTemplate()}>
