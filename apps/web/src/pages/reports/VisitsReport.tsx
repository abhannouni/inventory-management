...
export default function VisitsReport() {
  ...
  const handleFilter = async () => {
    ...
    if (fetchVisitsReport.rejected.match(res)) toast.error(res.payload as string || t('visitsReport.loadErrorToast'));
  };
    ...
    <div>
      <div className="filter-bar" style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignSelf: 'flex-end' }}>
          <Button onClick={handleFilter}>{t('filters.apply')}</Button>
          <Button variant="ghost" onClick={handleReset}>{t('filters.reset')}</Button>
        </div>
      </div>
              ...
              <div className="card-body">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                  <div>
                    ...
                    <p style={{ color: 'var(--gray-500)', fontSize: 13 }}>{formatDate(v.checkin_at, i18n.language)}</p>
                  </div>
                ...
                {v.summary && (
                  <div className="summary-grid">
                    <div className="summary-item">
                      ...
                      <div className="summary-label">{t('visitsReport.summary.total')}</div>
                    </div>
                    <div className="summary-item">
                      <div className="summary-value" style={{ color: 'var(--success)' }}>{v.summary.inStock}</div>
                      <div className="summary-label">{t('visitsReport.summary.inStock')}</div>
                    </div>
                    <div className="summary-item">
                      <div className="summary-value" style={{ color: 'var(--warning)' }}>{v.summary.lowStock}</div>
                      <div className="summary-label">{t('visitsReport.summary.lowStock')}</div>
                    </div>
                    <div className="summary-item">
                      <div className="summary-value" style={{ color: 'var(--danger)' }}>{v.summary.outOfStock}</div>
                      <div className="summary-label">{t('visitsReport.summary.outOfStock')}</div>
                    </div>
                  </div>
                )}
                ...
                {v.summary && v.summary.total > 0 && (
                  <div style={{ marginTop: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--gray-500)', marginBottom: 4 }}>
                      <span>{t('visitsReport.completionRate')}</span>
